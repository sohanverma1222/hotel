...
export async function GET(request: Request) {
  try {
    ...
    // If pagination is requested, use the paginated method
    if (paginated) {
      const result = await dbService.getGuestsPaginated(page, limit, search || undefined);
      
      // Enhanced guests with basic statistics
      const enhancedGuests = await Promise.all(
        result.guests.map(async (guest) => {
