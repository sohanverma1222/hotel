...
export default function BillingPage() {
  ...
  const handlePrintInvoice = (bill: any) => {
    ...
    const invoice = {
      id: bill.id,
      invoiceNumber: bill.invoiceNumber || `INV-${bill.id}`,
      date: bill.date || new Date().toISOString(),
      dueDate: bill.dueDate || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
      status: bill.status,
      guestName: bill.guestName,
      guestEmail: bill.guestEmail || 'guest@example.com',
      roomNumber: bill.roomNumber,
      checkIn: bill.checkIn || new Date().toISOString(),
      checkOut: bill.checkOut || new Date().toISOString(),
      items: bill.items || [
        {
          description: `Room ${bill.roomNumber} - Accommodation`,
          quantity: 1,
          unitPrice: bill.total || 0,
          totalPrice: bill.total || 0,
          category: 'room'
        }
      ],
      subtotal: bill.subtotal || (bill.total * 0.9) || 0,
      taxes: bill.taxes || (bill.total * 0.1) || 0,
      total: bill.total || 0
    };
  ...
  const handleDownloadAllInvoices = async () => {
    // In a real application, this would generate a ZIP file or multiple PDFs
    console.log('Downloading all invoices for', filteredBills.length, 'bills');
    alert(`Preparing download for ${filteredBills.length} invoices. This feature would generate a ZIP file with all invoices.`);
  };
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => handlePrintInvoice(bill)}>
                                <Download className="h-4 w-4 mr-2" />
                                Download PDF
                              </DropdownMenuItem>
                              <DropdownMenuItem>
